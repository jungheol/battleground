...
public class EffectTool : EditorWindow
{
    ...
    private void OnGUI() {
	    ...
	    {
		    UnityEngine.Object source = effectSource;
				    ...
				    SP2 = EditorGUILayout.BeginScrollView(this.SP2);
				    {
					    if (effectData.GetDataCount() > 0) {
						    EditorGUILayout.BeginVertical();
						    {
							    ...
							    effectData.effectClips[selection].effectType = (EffectType)EditorGUILayout.EnumPopup("이팩트 타입",
							    ...
							    effectSource = (GameObject)EditorGUILayout.ObjectField("이팩트", this.effectSource, typeof(GameObject),
