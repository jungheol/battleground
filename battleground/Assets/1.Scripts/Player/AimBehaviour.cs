...
public class AimBehaviour : GenericBehaviour {
	...
	private void Update() {
		...
		} else if (Input.GetAxisRaw(ButtonName.Aim) == 0 && isAim) {
			StartCoroutine(ToggleAimOff());
		}
		//조준중일때는 달리기를 하지 않습니다.
		canSprint = !isAim;
		if (Input.GetButtonDown(ButtonName.Shoulder) && isAim && !peekCorner) {
