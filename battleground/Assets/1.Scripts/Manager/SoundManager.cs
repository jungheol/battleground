...
public class SoundManager : SingletonMonobehaviour<SoundManager> {
    ...
    public AudioSource[] effect_aduios = null;
    ...
    void Start()
    {
        ...
        if (this.effect_aduios == null || this.effect_aduios.Length == 0) {
            this.effect_PlayStartTime = new float[effectChannelCount];
            this.effect_aduios = new AudioSource[effectChannelCount];
            for (int i = 0; i < effectChannelCount; i++) {
                ...
                this.effect_aduios[i] = effect.GetComponent<AudioSource>();
                this.effect_aduios[i].playOnAwake = false;
            }
        }
        ...
        if (this.mixer != null) {
            ...
            for (int i = 0; i < this.effect_aduios.Length; i++) {
                this.effect_aduios[i].outputAudioMixerGroup = mixer.FindMatchingGroups(EffectGroupName)[0];
            }
        }
    ...
    void PlayAduioSourceAtPoint(SoundClip clip, Vector3 position, float volume) {
    ...
    public bool IsDifferentSound(SoundClip clip) {
        ...
        if (currentSound != null && currentSound.readId == clip.readId && IsPlaying() &&
    ...
    public void PlayEffectSound(SoundClip clip) {
        ...
        for (int i = 0; i < this.effectChannelCount; i++) {
            if (this.effect_aduios[i].isPlaying == false) {
                PlayAudioSource(this.effect_aduios[i], clip, clip.maxVolume);
                this.effect_PlayStartTime[i] = Time.realtimeSinceStartup;
                isPlaySuccess = true;
                break;
            } else if (this.effect_aduios[i].clip == clip.GetClip()) {
                this.effect_aduios[i].Stop();
                PlayAudioSource(effect_aduios[i], clip, clip.maxVolume);
                this.effect_PlayStartTime[i] = Time.realtimeSinceStartup;
                isPlaySuccess = true;
                break;
            }
        ...
        if (isPlaySuccess == false) {
            ...
            PlayAudioSource(this.effect_aduios[selectIndex], clip, clip.maxVolume);
        }
    }
    ...
    public void PlayEffectSound(SoundClip clip, Vector3 position, float volume) {
        ...
        for (int i = 0; i < this.effectChannelCount; i++) {
            if (this.effect_aduios[i].isPlaying == false) {
                PlayAduioSourceAtPoint(clip, position, volume);
                this.effect_PlayStartTime[i] = Time.realtimeSinceStartup;
                isPlaySuccess = true;
                break;
            } else if (this.effect_aduios[i].clip == clip.GetClip()) {
